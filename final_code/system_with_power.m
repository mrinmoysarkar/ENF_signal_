...
for total_loop = 1:4
    ...
    if total_loop == 1 || total_loop == 2
        database = DATABASE{2};
        vth = 0.9;
        [trainning_matrix,group]=train_mat_generator(database,1,0); % only power in trainer
    else
        database = DATABASE{1};
    end
    ...
    for i=1:length(practice)
        ...
        hf = practice(i+j).HF;
        sample = [];
        if total_loop == 1 || total_loop == 2 % only power in trainer
        ...
        elseif total_loop == 2
            ...
            sample(1,:) = feature;
            [grid_name(i),p(i)]=classify_svm(sample,trainning_matrix,group,vth);
        elseif total_loop == 3 || total_loop == 4 % only power in trainer
            Vth = [.7 .7 .83 .8 .7];
            names = 'BHEDG';
            for p50=1:5
                sample = [];
                ...
                [trainning_matrix,group]=train_mat_generator(database,3,p50);% only power in trainer
